...
#include "hitechnic-irseeker-v2.c"//include Xander's IR driver
#include "constants.h"
...
task main()//start task main
{
	...
	while (true)//loop forever
	{
		...
		if(joy1Btn(9) == 1) {//if the right top back button (button 9) is pressed
			servo(armServo) = ARM_CLOSED;//put the arm servo at position 20 (up)
		}
		else if(joy1Btn(8) == 1) {//if the right bottom button (button 8) is pressed
			servo(armServo) = ARM_OPEN;//put the arm servo at position 195 (down)
		}
		...
		if(joy1Btn(5) == 1 && servo(armServo) < CLAW_HIGH_LIMIT) {//if the left top back button (button 5) is pressed and claw is not already fully open
				servo(armServo) -= 1;//open the claw
		}
			...
			servo(armServo) += 1;//close the claw
		}
